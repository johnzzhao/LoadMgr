/** 
** $Id: fullstimer_log_parser.sas,v 1.5 2015/04/03 15:11:26 jozhao Exp $
** Usage:
** The %LOGPARSE() macro extracts performance statistics from a SAS log file.
** read "Sample 34301: Parse SAS® Logs to Extract Performance and Timing Information " on http://support.sas.com/kb/34/301.html
**
*/
...
data _null_;
   ...
   log_dir = sysget("LOG_DIR");
   package_name = sysget("PACKAGE_NAME");
   retain_fullstimer_hist = sysget("RETAIN_FULLSTIMER_HIST");
   call symput('hostname', hostname);
   call symput('package_name', package_name);
   call symput('log_dir', log_dir);
   call symput('retain_fullstimer_hist', retain_fullstimer_hist);
run;

libname sas_perf "&log_dir";
%let package_name=%trim(&package_name);

%macro define_ds;
   %if &sysscpl = Linux and  %sysfunc(exist(sas_perf.&package_name._perf)) = 0 %then %do;
      data sas_perf.&package_name._perf;
         length hostname  $32 logfile   $200 stepcnt 4 stepname  $20 
                realtime  usertime  systime realsec cpusec realtime_vs_cputime osmem pageflt pagercl osvconsw  
                osiconsw  blkinput  bkoutput 8;
         format realtime usertime systime cputime time12.3
                realsec cpusec realtime_vs_cputime 8.2;
         label  
             host = 'System Name'
             logfile = 'Log file name'
             stepcnt = 'Step Number'
             stepname = 'Step Name'
             realtime = 'Elapsed Time'
             realsec  = 'Elapsed Time in Seconds'             
             systime = 'System Time'
             usertime = 'User Time'
             cpusec   = 'CPU Time in Seconds'
             realtime_vs_cputime = 'realtime / cputime'
             osmem = 'OS Memory Used'
             pageflt = 'Page Faults'
             pagercl = 'Page Reclaims'
             pageswp = 'Page Swaps'
             osiconsw = 'Involuntary Context Switches'
             osvconsw = 'Voluntary Context Switches'
             bkoutput = 'Block Output Operations'
             blkinput = 'Block Input Operations'
             obsin = 'Observations Read'
             obsout = 'Observations Written'
             varsout = 'Variables Written'
...
%logparse(&sysparm, &package_name._perf, OTH, , append=&retain_fullstimer_hist);

data &package_name._perf;
      length hostname  $32 logfile   $200 stepcnt 4 stepname  $20 
             realtime  usertime  systime realsec cpusec realtime_vs_cputime osmem pageflt pagercl osvconsw  
             osiconsw  blkinput  bkoutput 8;
      format realtime usertime systime cputime time12.3
                realsec cpusec realtime_vs_cputime 8.2;
      keep hostname logfile  stepcnt  stepname  realtime  usertime  systime realsec cpusec realtime_vs_cputime 
            osmem pageflt pagercl osvconsw  osiconsw  blkinput  bkoutput ;
   set &package_name._perf;
   realsec = realtime;
   cpusec =  usertime + systime;
   if cpusec ne 0 then realtime_vs_cputime = realsec/cpusec;
   else realtime_vs_cputime = .; 
   hostname = "&hostname";
run;

proc append base = sas_perf.&package_name._perf data = &package_name._perf;
run;
